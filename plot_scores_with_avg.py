...
for i, survey_name in enumerate(survey_names):
    ...
    # Calculate statistics for each follow-up number
    stats = patient_scores_over_time.groupby('follow_up_number')['score'].agg([
        'mean',
        lambda x: np.percentile(x, 25),  # Q1
        lambda x: np.percentile(x, 75)   # Q3
    ]).reset_index()
    stats.columns = ['follow_up_number', 'mean', 'q1', 'q3']
    ...
    # Plot mean line
    ax.plot(stats['follow_up_number'], stats['mean'], 
            color='black', 
            linewidth=2, 
            label='Mean Score')
